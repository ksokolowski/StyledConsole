...
def create_layout_from_config(config: dict[str, Any]) -> Any:
    ...
    Supported Types:
        ...
        - "banner": Figlet text (rendered as Text since we can't easily return ANSI string as renderable without wrapping).
          *Note*: For full fidelity banners, using the Console.banner() method directly is preferred, 
          but here we wrap it in a Panel or Text.
    ...
    if item_type == "group":
        ...
        return create_table_from_config(
...
def _create_group(config: dict[str, Any]) -> Group:
    ...
    for item_config in config.get("items", []):
        try:
            ...
            items.append(Text(f"Error: {str(e)}", style="bold red"))
