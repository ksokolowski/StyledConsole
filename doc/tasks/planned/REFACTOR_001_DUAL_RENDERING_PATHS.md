...
2. **Backward compatibility:** Existing code continues working through v0.4.x
3. **Clear migration:** Users transition smoothly with deprecation warnings
4. **Code reduction:** Eliminate ~400 LOC of duplicate logic
...
2. **Update `frame.py`:** Remove `FrameRenderer` class entirely, keep `Frame` dataclass
3. **Update `__init__.py`:** Remove `FrameRenderer` from exports
4. **Update tests:** Delete `test_frame_adapter.py`
