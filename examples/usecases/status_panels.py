...
""",
    title=f"{EMOJI.GREEN_CIRCLE} All Services Healthy",
    border="rounded",
    border_color="green",
    width=75,
)
...
console.rule(f"{EMOJI.WARNING} DEGRADED PERFORMANCE", style="yellow")
console.newline()
...
{EMOJI.WARNING} Application Server (Node.js)
...
""",
    title=f"{EMOJI.YELLOW_CIRCLE} Performance Degradation Detected",
    border="thick",
    border_color="yellow",
    width=75,
)
...
{EMOJI.WARNING} Database (PostgreSQL)
...
{EMOJI.CHECK_MARK_BUTTON} Cache (Redis)
  ...
  Hit rate: 94.2% | Memory: 847 MB / 2 GB
""",
    title=f"{EMOJI.RED_CIRCLE} Critical Service Failure",
    border="double",
    border_color="red",
    width=75,
)
...
{EMOJI.CARD_FILE_BOX} Disk Usage
  /: 342 GB / 500 GB (68.4%)
...
console.rule(f"{EMOJI.TEST_TUBE} APPLICATION HEALTH", style="green")
console.newline()
...
console.rule(f"{EMOJI.PACKAGE} KUBERNETES CLUSTER", style="magenta")
console.newline()
...
{EMOJI.PACKAGE} Pods (24 / 24 running)
...
console.rule(f"{EMOJI.CALENDAR} UPTIME TRACKING", style="cyan")
console.newline()
...
{EMOJI.CALENDAR} Historical Performance
  Last 7 days: 100.00%
  Last 30 days: 99.97%
  Last 90 days: 99.94%
  This year: 99.92%
""",
    title=f"{EMOJI.TROPHY} Uptime & Reliability",
    border="solid",
    border_color="cyan",
    width=75,
)
...
1. STATUS HIERARCHY
   ...
   {EMOJI.YELLOW_CIRCLE} Degraded: Yellow, warning, elevated metrics
   {EMOJI.RED_CIRCLE} Critical: Red, X mark, failure indicators
   {EMOJI.BLUE_CIRCLE} Unknown: Blue/gray, unknown state

2. KEY METRICS
   • Always show: Status, Uptime, Resource usage
   • Include thresholds: "42/100 connections"
   • Show trends: "normally 120ms"
   • Highlight issues: "{EMOJI.FIRE} HIGH" for alerts
...
{EMOJI.GREEN_CIRCLE} HEALTHY         All systems operational
{EMOJI.YELLOW_CIRCLE} DEGRADED        Performance issues, still working
{EMOJI.RED_CIRCLE} CRITICAL        Service down or failing
{EMOJI.BLUE_CIRCLE} UNKNOWN         Cannot determine status
{EMOJI.GEAR} STARTING        Service initializing
{EMOJI.WARNING} WARNING         Issues detected, monitoring
