...
from styledconsole import EMOJI, THEMES, Console, GradientSpec, Theme
...
def show_gradient_themes(console: Console) -> None:
    ...
    console.text(f"\n{EMOJI.RAINBOW} Gradient Themes:", bold=True, color="magenta")
    ...
    for theme in THEMES.gradient_themes():
        ...
        content = [
            f"{EMOJI.SPARKLES} Auto-applied gradients:",
...
def show_custom_gradient_theme(console: Console) -> None:
    ...
    console.text(f"\n{EMOJI.SPARKLES} Custom Gradient Theme:", bold=True, color="gold")
    console.text("")
...
def show_semantic_colors(console: Console) -> None:
    ...
    dark_console.frame(
        f"{EMOJI.WARNING} Warning message!",
        border="rounded",
