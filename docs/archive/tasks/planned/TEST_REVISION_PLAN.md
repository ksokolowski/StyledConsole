...
1. `test_colorize_with_named_color` - Tests `_colorize()` with CSS4 names
1. `test_colorize_content_in_line` - Tests `_colorize_content_in_line()` private method
1. `test_colorize_borders_in_line` - Tests `_colorize_borders_in_line()` private method
...
1. Snapshot tests need updating for new output format
1. Banner rendering now produces different (but still correct) widths

**Tests Failing:**

1. `test_banner_with_all_features` - Width consistency across features
1. `test_realistic_application_title` - Real-world banner width
1. `test_combined_features` - Combined features width check
...
1. Verify visually that banners look correct
1. Either:
   - Update expected widths if new behavior is correct
   - Update snapshots if using snapshot testing
   - Fix width calculation if genuinely broken
1. Consider if tests are too brittle (testing exact widths vs "reasonable width")
...
1. **Verification first:** We want to verify the core functionality works (659 tests passing) before removing safety nets.
1. **Visual validation:** Need to run examples and validate visual output before declaring tests obsolete.
1. **Holistic approach:** Phase 4 will handle all cleanup together (deprecated methods, tests, documentation).
