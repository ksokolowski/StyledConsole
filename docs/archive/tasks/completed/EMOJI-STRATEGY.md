...
1. **Width calculation:** Some emojis render as width=1, some width=2, some unpredictable
1. **Terminal support:** Modern terminals handle basic icons well, but struggle with compound emojis
1. **Alignment issues:** Complex emojis can break frame borders and text alignment
...
### Tier 3: ZWJ Sequences (❌ NOT SUPPORTED)
...
**Status:** ❌ **NOT SUPPORTED - No practical solution exists**

**Why They Cannot Be Supported:**

1. **Terminal rendering problem:** Terminals display ZWJ sequences as multiple separate emojis
1. **Unpredictable width:** Family emoji might render as 4 separate faces (width 8) instead of 1 family (width 2)
1. **No detection possible:** Cannot reliably detect how a terminal will render these
1. **Breaks frames:** Width miscalculation by 2-6 characters completely breaks border alignment
...
# What terminal displays:
👨👩👧 Family  # Three separate emojis, not one family glyph!
```

**Recommendation:** Use simple single-codepoint emojis instead:

| Instead of              | Use                     |
| ----------------------- | ----------------------- |
| 👨‍👩‍👧 (Family ZWJ)         | 👥 (Bust in Silhouette) |
| 👩‍💻 (Woman Technologist) | 💻 (Laptop)             |
| 🏳️‍🌈 (Rainbow Flag)       | 🌈 (Rainbow)            |
...
- ✅ Tier 1 basic icons fully supported
- ✅ VS16 emojis fully supported with automatic spacing
- ⚠️ Tier 2 modified emojis (skin tones) - terminal dependent
- ❌ Tier 3 ZWJ sequences - **explicitly not supported**

### Future Releases

- Focus on stability and terminal compatibility detection
- No plans to support ZWJ sequences (terminal rendering is fundamentally broken)
...
- ❌ Not supported, will break frames (use alternatives)
...
1. **Terminal fragmentation:** ZWJ support is inconsistent across terminals in 2025
1. **Library dependencies:** Full grapheme support requires heavier dependencies
1. **Testing burden:** Tier 2/3 require extensive cross-terminal testing
1. **User expectations:** Better to have reliable basics than buggy advanced features
