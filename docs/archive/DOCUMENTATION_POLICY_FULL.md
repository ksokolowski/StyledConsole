...
1. [Standards & Formatting Rules](#standards--formatting-rules)
...
Every document MUST begin with a standardized header block:

```markdown
# Document Title

**Purpose:** One-line description of what this document covers
**Audience:** Who should read this (Developers, Contributors, Users)
**Status:** Active (v0.4.0) | Draft | Archived
...
- Title uses `#` (H1), only one H1 per document
- Metadata uses `**bold:**` format with two trailing spaces for line breaks
- Status includes version number in parentheses
...
#### Standard Section Order for Guides

```markdown
## Overview          # Brief intro, what this covers
## Quick Start       # 3-5 line working example
## Quick Reference   # Table for scanning
## [Main Content]    # Organized by topic
## How It Works      # Technical explanation (optional)
## API Reference     # Function signatures (optional)
## Troubleshooting   # Common problems & solutions
## Version History   # Changes by version
## See Also          # Related docs and files
```
...
| Level     | Usage               | Example                    |
| --------- | ------------------- | -------------------------- |
| `#` H1    | Document title only | `# Emoji Support`          |
| `##` H2   | Major sections      | `## Quick Start`           |
| `###` H3  | Subsections         | `### Import`               |
| `####` H4 | Sub-subsections     | `#### Status & Indicators` |

**Rules:**

- Never skip levels (no H1 → H3)
- H2 sections separated by `---` horizontal rules
- No colons in headings (use `## Unsupported Emojis` not `## Unsupported: Emojis`)

______________________________________________________________________

### Horizontal Rules

**Use `---` consistently** (three hyphens)

**NOT:**

- ❌ `______________________________________________________________________` (underscores)
- ❌ `***` (asterisks)
- ❌ `===` (equals)
...
- After document header
- Before "See Also" section
...
- Use consistent column widths where practical
- For Yes/No columns, use: ✅ Full | ⚠️ Partial | ❌ None
...
|---------|---------|---------|-------|
| Feature A | ✅ Full | `example` | Additional info |
| Feature B | ⚠️ Varies | `example` | Caveats here |
| Feature C | ❌ None | `example` | Reason why |
```
...
**Supported languages:** `python`, `bash`, `text`, `markdown`, `json`, `yaml`

#### Code Example Pattern

Show import → usage → output:
...
console = Console()
console.frame("Content", title=f"{EMOJI.STAR} Title")
````
...
- Show expected output when visual
...
- ✅ Item one (supported)
- ✅ Item two (supported)
- ⚠️ Item three (partial)
- ❌ Item four (not supported)
````
...
- Use `-` for bullets (not `*`)
- Use status emojis for feature lists: ✅ ⚠️ ❌
...
| ------------- | --------- | ----------------------------- |
| Code/commands | Backticks | `visual_width()`              |
| File paths    | Backticks | `src/styledconsole/emojis.py` |
| Parameters    | Backticks | `start_color`                 |
| Emphasis      | Bold      | **important**                 |
| Terms         | Italic    | *VS16 selector*               |
| Keys          | Bold      | **Ctrl+C**                    |

**Rules:**

- Use `backticks` for anything typed or in code
- Use **bold** for emphasis and warnings
- Use *italic* sparingly for introducing terms

______________________________________________________________________

### Version References

**Current version:** Always reference actual version from `__init__.py`
...
|---------|------|---------|
| v0.4.0 | November 2025 | Strategy pattern, improved emoji handling |
| v0.3.0 | November 2025 | VS16 support with automatic spacing |
...
```markdown
See [EMOJI_SUPPORT.md](guides/EMOJI_SUPPORT.md) for details.
```
...
- **Reference:** `doc/reference/EMOJI_CONSTANTS.md`
```

**Rules:**

- Use relative paths from doc/ folder
- Bold the category (Examples, Source, Reference)
...
### Problem title as question or symptom
...
- H3 for each problem
- Always include Symptom, Cause, Solution
- Numbered steps for multi-step solutions
...
| Category | Header Status | Has Quick Start | Has Troubleshooting |
|----------|---------------|-----------------|---------------------|
| Guide | Active (vX.X) | ✅ Required | ✅ Required |
| Reference | Complete | ❌ Optional | ❌ Optional |
| Task/Plan | Planning/In Progress | ❌ No | ❌ No |
| Notes | Date only | ❌ No | ❌ No |
...
| `______` horizontal rules | `---` horizontal rules |
| Skip heading levels | Use H2 → H3 → H4 in order |
| Generic examples (`foo`, `bar`) | Real-world examples |
| Undated "Last Updated" | Include actual date |
| Version without checking code | Verify in `__init__.py` |
| Dense paragraphs | Break into lists and tables |
...
- [ ] Header has Purpose, Audience, Status, Last Updated
- [ ] Status version matches `__init__.py`
- [ ] All H2 sections separated by `---`
- [ ] No skipped heading levels
- [ ] All code blocks have language specified
- [ ] All code examples tested and working
- [ ] Tables have header rows
- [ ] Lists have blank line before them
- [ ] Internal links use relative paths
- [ ] Troubleshooting follows Symptom/Cause/Solution pattern
